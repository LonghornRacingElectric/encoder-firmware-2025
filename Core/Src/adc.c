...
void HAL_ADC_MspInit(ADC_HandleTypeDef* adcHandle) {
    ...
    if (adcHandle->Instance == ADC1) {
        ...
        */
        ...
        GPIO_InitStruct.Pin = STEER_VGMR_Pin | BSPD_Brake_Analog_Pin |
                              BSE1_fused_Pin | BSE2_fused_Pin | APPS1_0_Pin;
        GPIO_InitStruct.Mode = GPIO_MODE_ANALOG;
        GPIO_InitStruct.Pull = GPIO_NOPULL;
        HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

        GPIO_InitStruct.Pin = BPPS2_O_Pin;
        GPIO_InitStruct.Mode = GPIO_MODE_ANALOG;
        GPIO_InitStruct.Pull = GPIO_NOPULL;
        HAL_GPIO_Init(BPPS2_O_GPIO_Port, &GPIO_InitStruct);
...
void HAL_ADC_MspDeInit(ADC_HandleTypeDef* adcHandle) {
    if (adcHandle->Instance == ADC1) {
        ...
        */
        ...
        HAL_GPIO_DeInit(GPIOA, STEER_VGMR_Pin | BSPD_Brake_Analog_Pin |
                                   BSE1_fused_Pin | BSE2_fused_Pin |
                                   APPS1_0_Pin);

        HAL_GPIO_DeInit(BPPS2_O_GPIO_Port, BPPS2_O_Pin);
...
float adc_getSin_N() { return ((float)adc_values[0]) / 4096.0f * 3.3f; }
float adc_getSin_P() { return ((float)adc_values[1]) / 4096.0f * 3.3f; }
float adc_getSin() {
    return ((float)adc_values[0] - (float)adc_values[1]) / 4096.0f * 3.3f;
}
// Get Cosine differential output through single ended ADC pins
float adc_getCosN() { return ((float)adc_values[2]) / 4096.0f * 3.3f; }
float adc_getCosP() { return ((float)adc_values[3]) / 4096.0f * 3.3f; }
float adc_getCos() {
